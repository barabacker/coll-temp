...
## 8. Реализация: отклонения от дизайна (обновлено по факту)

Живая проверка всех 5 площадок вскрыла, что «единый шаблон» — упрощение.
Финальная реализация (коммит `ba42a6e`) отличается от §3 так:

- **Два шаблона карточки листинга**, не один:
  - **A** — карточка = торги, `<a class="block-lot" href>` (trade_alliance,
    electro_torgi, seltim). У `seltim` внутри `<a>` вложены ссылки, которые
    lxml вырезает → номер/заголовок там — обычные `<span>`, не `<a>`.
  - **B** — карточка = лот, `<div class="block-lot">` с `green-text`-ссылками
    (torgi82, vetp); дедуплицируется до торгов.
- **`parse_listing` унифицирован:** карточка ищется тег-независимо
  (`//*[block-lot]`), номер торгов — по `bold`-элементу, чей текст матчит
  `^\d+[–-]` (текст или вложенный `<a>`); detail-URL — из `@href` карточки либо
  ссылки номера; результат дедуплицируется по `trade_id`.
- **Торговые fingerprint-поля (`trade_title`/даты) берутся из `#main-info`
  detail-страницы**, а не из карточки листинга (на шаблоне B карточка
  лот-уровня). `status`/`price` — per-lot из `#lots`.
- **`parse_lots`** — тег-независимый блок (`<a>`/`<div>`), ссылка лота по
  `/lots/` в href (цвето-независимо).
- **Верификация:** все 5 площадок проверены вживую end-to-end (curl_cffi
  chrome-impersonation, `main.py <key> 1`) — собирают лоты с непустыми
  `price`/`status`/`lot_id`. Фикстуры-регресс: `listing_page1`/`detail_oaof`
  (A-anchor), `seltim_listing` (A-span), `torgi82_listing`/`torgi82_detail` (B).

**Остаточный follow-up (не блокер):** `parse_main_info` — плоский dict
«last-wins»; на detail дважды встречается лейбл «Фамилия, имя, отчество» (арб.
управляющий и должник) → одно из имён теряется в reference-блобе `item['detail']`
(в fingerprint не участвует). Починка — сделать `main_info` вложенным по
секциям, если downstream нужна полнота этого блоба.
