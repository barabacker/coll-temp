...
Split into separate services only when operational, ownership, scaling,
deployment, data ownership, security, or failure-boundary reasons justify it.
...
Domain rules should not depend on the web framework, database sessions, HTTP
clients, queues, or framework lifecycle.

## Suggested Python/FastAPI structure

Adapt to repository conventions when they exist.
...
- mixing validation, persistence, HTTP and business rules in one function
...
Split services only for real operational, ownership, scaling, deployment, data,
security, or failure-boundary reasons.
