...
from tenders.core.parsing import is_active_status, read_only_active
from tenders.core.storage.contracts import lot_fingerprint
from tenders.sources.fogsoft.inprotect import solve_inprotect
from tenders.sources.fogsoft.parsing.detail import (
    parse_attachments,
    parse_detail_sections,
    parse_price_schedule,
)
from tenders.sources.fogsoft.parsing.tables import (
    ajax_headers,
    find_next_ctl,
    parse_table,
    read_max_pages,
)
from tenders.sources.fogsoft.parsing.viewstate import (
    build_payload,
...
class TenderFogsoft(LotParser):
    ...
    async def parse(self, response: Response) -> AsyncIterator[Request | dict[str, Any]]:
        ...
        if self.ctx.sink is not None:
            ...
            if lot_ids:
                existing_fingerprints = await self.ctx.sink.get_fingerprints(self.name, lot_ids)
